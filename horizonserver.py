...
logging.basicConfig(level=logging.INFO, format="%(asctime)-15s| %(threadName)-18s| %(message)s", stream=sys.stdout)


class HorizonServer:
    """
    REST API Client for Horizon Connection Server, this is the only public class, create an instance first
    """
    ...
    def http_get(self, url, params=None, headers=None, verify=False, timeout=None, **kwargs) -> requests.Response:
        if headers is None:
            headers = self._default_headers
        response = requests.get(url=url, params=params, headers=headers, verify=verify, timeout=timeout, **kwargs)
    ...
    def http_delete(self, url, params=None, headers=None, verify=False, timeout=None, **kwargs) -> requests.Response:
        if headers is None:
            headers = self._default_headers
        response = requests.delete(url=url, params=params, headers=headers, verify=verify, timeout=timeout, **kwargs)
        if response.status_code != 204:
    ...
    def _login(self):
        ...
        data = {"username": self._username, "password": self._password, "domain": self._domain}
...
class _GSSAPIAuthenticator:
    ...
    def __init__(self, gssapi_authenticator_id, horizon_server: HorizonServer):
        ...
        self.horizon_server = horizon_server
        self.url = 'https://{}/rest/config/v1/gssapi-authenticators/{}'.format(horizon_server.address, self.id)
    ...
    def enable_login_as_current_user(self, enabled=True):
        ...
        response = self.horizon_server.http_put(self.url, json=self.json_put_data)
        if response.status_code == 204:
            # sync data change
...
class _ConnectionServer:
    ...
    def __init__(self, connection_server_id, horizon_server: HorizonServer):
        ...
        self.horizon_server = horizon_server
        self.url = 'https://{}/rest/config/v1/connection-servers/{}'.format(horizon_server.address, self.id)
        self._generate_get_response_data()
